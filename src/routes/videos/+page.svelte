<script lang="ts">
    ...
    const heroVideo = {
        title: "青山｜畢展影音紀錄與精華",
        description:
            "用影像保存文化現場，用紀錄回應時間在這裡，我們用鏡頭記錄艋舺的重要文化記憶，包含遶境實況、宮廟專訪、傳統儀式與團隊製作過程。",
    ...
    const CARD_BASE =
        "flex-shrink-0 cursor-pointer group relative rounded-lg overflow-hidden transition-all duration-300 ease-out bg-zinc-900 shadow-lg hover:shadow-2xl hover:scale-[1.02] md:hover:scale-105 hover:z-20 border border-white/5";

    const celebrateImages: ContentItem[] = Array.from(
        { length: 44 },
        (_, i) => ({
            id: 201 + i,
            title: "",
            url: "#",
            typeLabel: "圖片",
            imageUrl: `/images/celebrate (${i + 1}).webp`,
            layout: "tall", // 這裡設定 layout，但會被 MULTI_ROW_CLASSES 覆蓋
            isImageOnly: true,
        }),
    );

    const pilImagesDay2: ContentItem[] = Array.from(
        { length: 300 },
        (_, i) => ({
            id: 301 + i,
            title: "",
            url: "#",
            typeLabel: "圖片",
            imageUrl: `/images/pilgrimage-day2 (${i + 1}).webp`,
            layout: "tall", // 這裡設定 layout，但會被 MULTI_ROW_CLASSES 覆蓋
            isImageOnly: true,
        }),
    );
    ...
    const pilImagesDay1: ContentItem[] = Array.from({ length: 96 }, (_, i) => ({
        ...
        url: "#",
        typeLabel: "圖片",
        imageUrl: `/images/pilgrimage-day1 (${i + 1}).webp`,
        layout: "tall", // 這裡設定 layout，但會被 MULTI_ROW_CLASSES 覆蓋
        isImageOnly: true,
    }));
    ...
    const pilImagesDay3: ContentItem[] = Array.from({ length: 96 }, (_, i) => ({
        ...
        url: "#",
        typeLabel: "圖片",
        imageUrl: `/images/pilgrimage-day3 (${i + 1}).webp`,
        layout: "tall", // 這裡設定 layout，但會被 MULTI_ROW_CLASSES 覆蓋
        isImageOnly: true,
    }));
            ...
            items: [
                {
                    id: 101,
                    title: "圖片合集",
                    url: "https://drive.google.com/drive/folders/1Wi6cqMp36W5WebrQ97YbQjJEUwojHccn",
                    typeLabel: "活動圖集",
                    imageUrl: "/images/exhi-cover.webp",
                    layout: "wide",
                    isImageOnly: true,
                },
                {
                    ...
                    title: "精華影片",
                    typeLabel: "影片",
                    imageUrl: "/images/school-cover.webp",
                    url: "https://youtube.com/shorts/JPzuTYox5gg?feature=share",
                    layout: "square",
                    isImageOnly: false,
                },
            ],
    ...
    const randomImagePath = `/images/celebrate (${randomNumber}).webp`;
</script>

<main
    class="bg-[#0a0a0a] min-h-screen text-gray-100 font-sans selection:bg-red-900 selection:text-white pb-20 font-bakudai-md"
>
    <section class="relative h-[85vh] md:h-[95vh] w-full overflow-hidden">
        <div class="absolute inset-0 z-0">
            <div
                class="w-full h-full bg-cover bg-center transition-transform duration-[10s] hover:scale-105"
                style="background-image: url('{randomImagePath}');"
            ></div>
            <div
                class="absolute inset-0 bg-gradient-to-t from-[#0a0a0a] via-black/40 to-transparent"
            ></div>
            <div
        ...
        <div
            class="absolute bottom-0 left-0 w-full p-6 md:p-12 lg:p-20 z-10 flex flex-col items-start"
        >
            <h1
                class="text-4xl sm:text-5xl md:text-7xl font-black mb-4 md:mb-6 leading-tight max-w-4xl drop-shadow-2xl text-transparent bg-clip-text bg-gradient-to-br from-white to-gray-400"
            >
                {heroVideo.title}
            </h1>
            <p
                class="text-sm sm:text-base md:text-lg text-gray-300 mb-8 max-w-xl leading-relaxed drop-shadow-md line-clamp-3 md:line-clamp-none"
            >
    ...
    <div class="relative z-20 space-y-12 md:space-y-20 -mt-10 md:-mt-20 pb-20">
        {#each contentCategories as category, index}
            <section class="pl-4 sm:pl-8 md:pl-16 lg:pl-20 animate-fade-in-up">
                <div
                    class="mb-4 md:mb-6 pr-4 group cursor-pointer inline-block"
                >
                    <h2
                        class="text-xl sm:text-2xl md:text-3xl font-bold text-white flex items-center gap-2 group-hover:text-red-500 transition-colors"
                    >
                        {category.title}
                        <span
                            class="text-xs sm:text-sm font-normal text-gray-500 group-hover:text-red-400/70 transition-colors mt-1 block sm:inline ml-0 sm:ml-2"
                        >
                            {category.subtitle}
                        </span>
                    </h2>
                        ...
                        {#each category.items as item}
                            ...
                            >
                                ...
                                >
                                    ...
                                    />
                                    <div
                                        class="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent opacity-60 group-hover:opacity-80 transition-opacity"
                                    ></div>
                                    ...
                                    {#if !item.isImageOnly}
                                        ...
                                        >
                                            <div
                                                class="w-12 h-12 md:w-16 md:h-16 rounded-full bg-red-600/90 flex items-center justify-center shadow-lg transform scale-50 group-hover:scale-100 transition-transform duration-300"
                                            >
                                                <svg
                                                    xmlns="http://www.w3.org/2000/svg"
                                                    class="h-6 w-6 md:h-8 md:w-8 text-white ml-1"
                                                    viewBox="0 0 20 20"
                                                    fill="currentColor"
                                                    ><path
                                                        fill-rule="evenodd"
                                    ...
                                    {#if item.isImageOnly}
                                        ...
                                        >
                                            <svg
                                                xmlns="http://www.w3.org/2000/svg"
                                                class="h-8 w-8 text-white drop-shadow-md transform translate-y-4 group-hover:translate-y-0 transition-transform"
                                                fill="none"
                                                viewBox="0 0 24 24"
                                                stroke="currentColor"
                                                ><path
                                    ...
                                    <div
                                        class="absolute bottom-0 left-0 w-full p-3 md:p-4 translate-y-2 group-hover:translate-y-0 transition-transform duration-300"
                                    >
                                        <div
                                            class="flex items-center gap-2 mb-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-75"
                                        >
                                            <span
                                                class="text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded-sm {item.isImageOnly
                                        ...
                                        {#if item.title}
                                            <h3
                                                class="text-sm md:text-base font-bold text-white leading-tight drop-shadow-md line-clamp-2"
                                            >
                                                {item.title}
                                            </h3>
                                        {/if}
