<script lang="ts">
    ...
    const contentCategories: Category[] = [
        ...
        },
        {
            ...
            subtitle: '凝視信仰',
            items: [
                { id: 101, title: '夜訪青山王', url: '#', typeLabel: '攝影', imageUrl: 'https://picsum.photos/600/400?random=20', layout: 'wide', isImageOnly: true },
                { id: 102, title: '陣頭風采', url: '#', typeLabel: '攝影', imageUrl: 'https://picsum.photos/400/400?random=21', layout: 'square', isImageOnly: true },
        ...
        },
        {
            ...
            subtitle: '雕梁畫棟的藝術細節',
            items: [
                { id: 201, title: '藻井特寫', url: '#', typeLabel: '建築', imageUrl: 'https://picsum.photos/500/300?random=30', layout: 'wide', isImageOnly: true },
                { id: 202, title: '龍柱細節', url: '#', typeLabel: '建築', imageUrl: 'https://picsum.photos/300/500?random=31', layout: 'tall', isImageOnly: true },
            ]
        }
    ];
        ...
        {#each contentCategories as category}
            <section class="pl-4 sm:pl-8 md:pl-16 lg:pl-20 animate-fade-in-up">
                ...
                <div class="relative group/slider">
                    <div class="flex overflow-x-auto space-x-4 md:space-x-6 pb-8 pr-8 custom-scrollbar scroll-smooth snap-x snap-mandatory">
                        {#each category.items as item}
                            ...
                            >
                                <div class="w-full h-full relative overflow-hidden">
                                    ...
                                    {#if !item.isImageOnly}
                                        <div class="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 bg-black/40 backdrop-blur-[2px]">
                                            <div class="w-12 h-12 md:w-16 md:h-16 rounded-full bg-red-600/90 flex items-center justify-center shadow-lg transform scale-50 group-hover:scale-100 transition-transform duration-300">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 md:h-8 md:w-8 text-white ml-1" viewBox="0 0 20 20" fill="currentColor">
                                                    <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-2a1 1 0 000-1.664l-3-2z" clip-rule="evenodd" />
                                                </svg>
                                            </div>
                                        </div>
                                    ...
                                    {#if item.isImageOnly}
                                         <div class="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 bg-black/20 backdrop-blur-[1px]">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-white drop-shadow-md transform translate-y-4 group-hover:translate-y-0 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    ...
                                    <div class="absolute bottom-0 left-0 w-full p-3 md:p-4 translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                                        <div class="flex items-center gap-2 mb-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-75">
                                            <span class="text-[10px] uppercase font-bold tracking-wider px-2 py-0.5 rounded-sm 
                                                {item.isImageOnly ? 'bg-blue-600/80 text-white' : 'bg-red-600/80 text-white'}">
                                                {item.typeLabel}
                                            </span>
                                        </div>
                                        <h3 class="text-sm md:text-base font-bold text-white leading-tight drop-shadow-md line-clamp-2">
                                            {item.title}
                                        </h3>
                                    </div>
                                </div>
